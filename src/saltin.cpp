...
void usage() {
	...
	puts("     a- Alison, c- Chris, h- Hadley, p- Peter Cahill, r- Ron Gillam, s- Sophie");
	...
	switch(*f++)
	{
		case 'c':
			if (*f == 'a' || *f == 'A') {
				...
				fprintf(stderr, "Please choose one of for +c option: 'a', 'c', 'p', 'r' or 's'\n");
				saltin_clean_up();
				clearcodes();
				cutt_exit(0);
			}
...
*/
			...
			if (isalpha(*(line+1)) || isalpha(*(line-1)) || *(line-1) == '^' || *(line-1) == '-' || sb)
				...
				*line = tchr;
				strcpy(pos,line);
				uS.shiftright(pos,strlen(saltin_templine));
...
static void processPartComm(char *part, char *comm) {
	...
	i = strlen(age);
	if (i > 0 && age[i-1] != '.' && strchr(age, '.') == NULL)
		strcat(age, ".");
...
//			printout("%tim:", line, NULL, NULL, TRUE);
			...
			if (coding == 0 || coding == 4 || coding == 5 || coding == 6) {
				if (part[0] != EOS) {
					processPartComm(part, comment);
				}
			}
			...
			if (*s == '|') {
				...
				if (*s == 'G' || *s == 'g') {
					if (pos > 4 && uS.isConsonant(st+pos-3) && uS.isVowel(st+pos-4) && uS.isConsonant(st+pos-5)) {
						...
						if (pos > 3 && (st[pos-4] == 'E' || st[pos-4] == 'e')) {
						} else {
...
static void cleanUpQuotes(char *line) {
	...
	for (i=0; line[i] != EOS; i++) {
		if (line[i] == (char)0xEF && line[i+1] == (char)0x81 && line[i+2] == (char)0xBC) {
			for (j=i+3; isSpace(line[j]); j++) ;
			if (j > i + 1)
				strcpy(line+i+1, line+j);
			line[i] = '|';
		} else if (line[i] == '`' || line[i] == (char)0xB4 || line[i] == (char)0xD5) {
			line[i] = '\'';
...
void call() {
	...
	while (fgets_cr(spareTier3, UTTLINELEN, fpin)) {
		...
		if (spareTier3[0] == '$')
			break;
		if (!uS.isUTF8(spareTier3) && !uS.partcmp(spareTier3, FONTHEADER, FALSE, FALSE)) {
			lnum++;
		}
	}
	if (spareTier3[0] != '$') {
