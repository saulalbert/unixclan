...
static int  targc;
static char **targv;
static char ftime, isSpeakerNameGiven, isRemoveRetracedCodes, isMORFound, morTierSelected;
...
static void script_process_tier(struct script_fname *fnameP, int uttNum) {
	...
	} else if (!morTierSelected && uS.partcmp(utterance->speaker,"%mor:",FALSE,FALSE)) {
		isMORFound = TRUE;
		if (uttNum <= 0) {
			fprintf(stderr,"Internal error: uttNum <= 0.\n");
			fprintf(stderr,"Quiting program.\n");
...
static void script_pr_result(void) {
	...
	for (fn=files_root; fn != NULL; fn=fn->nextFname) {
		outputStringForExcel(fpout, fn->fname, 0);
...
void call() {
	...
	if (!combinput)
		script_pr_result();
	if (!isMORFound && !R6 && !morTierSelected) {
#ifdef UNX
