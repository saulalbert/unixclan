...
static char puncts(long ti) {
	if (uS.mStrnicmp(trn+ti, "end|", 4) == 0 || uS.mStrnicmp(trn+ti, "beg|", 4) == 0 || uS.mStrnicmp(trn+ti, "cm|", 3) == 0 ||
		uS.mStrnicmp(trn+ti, "bq|", 3) == 0  || uS.mStrnicmp(trn+ti, "eq|", 3) == 0)
		return(TRUE);
	return(FALSE);
}

static void compairTiers(long ln, char isMorFirst, long *total, long *mismatched) {
...
void call(void) {
		...
		if (RightTier) {
 			if (utterance->speaker[0] == '*' || uS.partcmp(utterance->speaker, "@End", FALSE, FALSE)) {
				if (trn[0] != EOS && mor[0] != EOS) {
					compairTiers(ln, isMorFirst, &total, &mismatched);
	...
	fprintf(fpout,"\nTotal number of items on %%trn:\t%ld\n", total);
