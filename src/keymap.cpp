...
static void pr_Excel(FILE *StatFpOut, EXCELCOL *cols) {
	int rowCount;
	EXCELCOL *col;
	KEYWDROW *twdr;
	EXCELROW *tr;
	...
	for (rowCount=0; rowCount < ExcelCnt; rowCount++) {
		...
		if (col->keyrows != NULL && col->keyrows->rowCnt == rowCount) {
			outputStringForExcel(StatFpOut, col->keyrows->fname, 1);
			outputStringForExcel(StatFpOut, col->keyrows->sp, 1);
			outputStringForExcel(StatFpOut, col->keyrows->keyword, 1);
			fprintf(StatFpOut, "%d", col->keyrows->count);
			twdr = col->keyrows;
