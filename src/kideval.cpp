...
//  kideval -c1 '+t*CHI'
// /WORK/clanW/src/unix/bin/kideval -c1 '+t*CHI' -L/WORK/clanW/src/unix/bin/kideval_lib
// /WORK/clanW/src/unix/bin/kideval '+t*CHI' -re -leng -L/WORK/clanW/src/unix/bin/kideval_lib *.cha
...
struct kideval_speakers {
	...
	float mWords100, morf100, mUtt100;
...
struct database {
	...
	float mUtt100_sqr, mUtt100, mn_mUtt100, mx_mUtt100, mUtt100_num;
	float mWords100_sqr, mWords100, mn_mWords100, mx_mWords100, mWords100_num;
	float morf100_sqr, morf100, mn_morf100, mx_morf100, morf100_num;
...
#endif
static int  kideval_SpecWords, DBGemNum, MinUttLimit, wdOffset;
...
void usage() {
	...
	mainusage(FALSE);
#ifdef UNX
	printf("CURRENT DATABASE COMPARISONS ARE PRELIMINARY AND SHOULD NOT BE USED FOR CLINICAL DIAGNOSIS.\n");
	printf("PLEASE SEE SLP MANUAL FOR RECOMMENDED INTERPRETATION.\n\n");
#else
	printf("%c%cCURRENT DATABASE COMPARISONS ARE PRELIMINARY AND SHOULD NOT BE USED FOR CLINICAL DIAGNOSIS.%c%c\n", ATTMARKER, error_start, ATTMARKER, error_end);
...
static void init_db(struct database *p) {
	...
	p->mUtt100_sqr = 0.0; p->mUtt100 = 0.0; p->mn_mUtt100 = 0.0; p->mx_mUtt100 = 0.0; p->mUtt100_num = 0.0;
	p->mWords100_sqr = 0.0; p->mWords100 = 0.0; p->mn_mWords100 = 0.0; p->mx_mWords100 = 0.0;  p->mWords100_num = 0.0;
	p->morf100_sqr = 0.0; p->morf100 = 0.0; p->mn_morf100 = 0.0; p->mx_morf100 = 0.0; p->morf100_num = 0.0;
...
static void kideval_process_tier(struct kideval_speakers *ts, struct database *db, char *rightIDFound, char *IDText) {
	...
	long stime, etime;
	double tNum;
	float mWords, morf, mUtt, mWords100, morf100, mUtt100;
...
static void retrieveTS(struct kideval_speakers *ts, char *line, struct database *db) {
	...
	line = extractFloatFromLine(line, &ts->mWords100, db);
	line = extractFloatFromLine(line, &ts->morf100, db);
	line = extractFloatFromLine(line, &ts->mUtt100, db);
...
static void prDebug(struct kideval_speakers *ts, char *IDText, char *fileName) {
	...
	fprintf(dbResults, "mWords100=%.0f\n", ts->mWords100);
	fprintf(dbResults, "morf100=%.0f\n", ts->morf100);
	fprintf(dbResults, "mUtt100=%.0f\n", ts->mUtt100);
...
//	format = "%f ";
	...
	fprintf(dbfpout, format, ts->mWords100);
	fprintf(dbfpout, format, ts->morf100);
	fprintf(dbfpout, format, ts->mUtt100);
...
static void OpenDBFile(FILE *fp, struct database *db, char *isDbSpFound) {
	...
	if (templineC[0] != 'V' || !isdigit(templineC[1])) {
		...
		fprintf(stderr, "Please download new database with \"File->Get KIDEVAL Database\" menu\n");
		kideval_error(db, FALSE);
	}
	t = atoi(templineC+1);
				...
				*isDbSpFound = TRUE;
				...
				if (ts->tUtt >= 50.0) {
					...
					if (ts->mUtt100 >= 100.0) {
						...
						ts->mWords100 = ts->mWords100 / ts->mUtt100;
						db->mWords100_num++;
						db->mWords100_sqr = db->mWords100_sqr + (ts->mWords100 * ts->mWords100);
						db->mWords100 = db->mWords100 + ts->mWords100;
						if (db->mn_mWords100 == (float)0.0 || db->mn_mWords100 > ts->mWords100)
							db->mn_mWords100 = ts->mWords100;
						if (db->mx_mWords100 < ts->mWords100)
							db->mx_mWords100 = ts->mWords100;
					...
					if (strcmp(ipsyn_lang, IPSYNNOCOMPUTE) != 0 && ts->ipsynUttCnt >= 100.0) {
						db->ipsynUttCnt_num++;
...
static void kideval_pr_result(void) {
	...
	excelCommasStrCell(fpout, "Total Utts");
	excelCommasStrCell(fpout, "MLU Utts,MLU Words,MLU Morphemes,MLU100 Utts,MLU100 Words,MLU100 Morphemes");
	excelCommasStrCell(fpout, "FREQ types,FREQ tokens,FREQ TTR");
	excelCommasStrCell(fpout, "NDW/100");
	excelCommasStrCell(fpout, "VOCD D_optimum_average");
	excelCommasStrCell(fpout, "Verbs/Utt");
	if (DBKeyRoot == NULL) { // exclude TD columns
		excelCommasStrCell(fpout, "TD Words,TD Utts,TD Time (secs),TD Words/Time,TD Utts/Time");
	}
	if (isRawVal) {
		excelCommasStrCell(fpout, "Word Errors");
	} else {
		excelCommasStrCell(fpout, "%% Word Errors");
	}
	excelCommasStrCell(fpout, "Utt Errors");
	excelCommasStrCell(fpout, "retracing[//],repetition[/]");
	if (strcmp(rulesfile, DSSNOCOMPUTE) != 0) {
		excelCommasStrCell(fpout, "DSS Utts,DSS");
	}
	if (strcmp(ipsyn_lang, IPSYNNOCOMPUTE) != 0) {
		excelCommasStrCell(fpout, "IPSyn Utts,IPSyn Total");
	}
...
// ".xls"
		...
		excelNumCell(fpout, "%.0f", ts->mUtt100);
		if (ts->mUtt100 < 100.0)
			excelStrCell(fpout, "N/A");
		else
			excelNumCell(fpout, "%.3f", ts->mWords100/ts->mUtt100);
		if (ts->mUtt100 < 100.0)
			excelStrCell(fpout, "N/A");
		else
			excelNumCell(fpout, "%.3f", ts->morf100/ts->mUtt100);
		...
		for (col=labelsRoot; col != NULL; col=col->next_label) {
			if (ts->mor_count == NULL) {
				...
				if (!isRawVal) {
					...
					excelNumCell(fpout, "%.0f", ts->mor_count[col->num]);
			}
		}
		excelRow(fpout, ExcelRowEnd);
		if (DBKeyRoot != NULL) {
...
// for mUtt100 always say N/A
			excelStrCell(fpout, "N/A");
			SD[SDn++].stars = 0;
//			compute_SD(&SD[SDn++], ts->mUtt100,  NULL, gdb->mUtt100_sqr, gdb->mUtt100, gdb->mUtt100_num);
			if (ts->mUtt100 < 100.0) {
				set_SD2NA(&SD[SDn++], gdb->mWords100_sqr, gdb->mWords100, gdb->mWords100_num);
				set_SD2NA(&SD[SDn++], gdb->morf100_sqr, gdb->morf100, gdb->morf100_num);
			} else {
				compute_SD(&SD[SDn++], ts->mWords100,  &ts->mUtt100, gdb->mWords100_sqr, gdb->mWords100, gdb->mWords100_num);
				compute_SD(&SD[SDn++], ts->morf100,  &ts->mUtt100, gdb->morf100_sqr, gdb->morf100, gdb->morf100_num);
			}
...
*/
	...
	if (DBKeyRoot != NULL) {
		...
		if (gdb->mUtt100_num <= 0.0)
			excelStrCell(fpout, "N/A");
		else
			excelNumCell(fpout, "%.3f",gdb->mUtt100/gdb->mUtt100_num);
		if (gdb->mWords100_num <= 0.0)
			excelStrCell(fpout, "N/A");
		else
			excelNumCell(fpout, "%.3f",gdb->mWords100/gdb->mWords100_num);
		if (gdb->morf100_num <= 0.0)
			excelStrCell(fpout, "N/A");
		else
			excelNumCell(fpout, "%.3f",gdb->morf100/gdb->morf100_num);
...
*/
		...
		excelNumCell(fpout, "%.0f",gdb->mUtt100_num);
		excelNumCell(fpout, "%.0f",gdb->mWords100_num);
		excelNumCell(fpout, "%.0f",gdb->morf100_num);
...
*/
	...
	if (sp_head != NULL) {
		if (DBKeyRoot != NULL) {
			excelRowOneStrCell(fpout, ExcelRedCell, "CURRENT COMPARISONS ARE PRELIMINARY AND SHOULD NOT BE USED FOR CLINICAL DIAGNOSIS.");
...
static char isEXCPostCode(char *sp, char *line, char *word) {
	int i;
...
*/
			if (!checktier(utterance->speaker)) {
				...
				if (*utterance->speaker == '*') {
					if (isEXCPostCode(utterance->speaker, utterance->line, word))
						lRightspeaker = FALSE;
					else
						lRightspeaker = TRUE;
				}
...
static void processEng(void) {
	...
	processScriptLine("Eng", 0, "+|cop -|*,~|cop	\"cop|*\"");
	processScriptLine("Eng", 0, "+|det:art		\"det:art\"");
	processScriptLine("Eng", 0, "+-PAST		\"*-PAST\"");
	processScriptLine("Eng", 0, "+-3S		\"*-3S\"");
	processScriptLine("Eng", 0, "+&3S		\"*&3S\"");
	processScriptLine("Eng", 0, "+|aux -|*,~|aux	\"aux|*\"");
	processScriptLine("Eng", 0, "+|*,~|cop	\"~cop|*\"");
	processScriptLine("Eng", 0, "+|*,~|aux	\"~aux|*\"");
}
...
static void processFra(void) {
	...
	processScriptLine("Fra", 0, "+&1s	\"1s\"");
	processScriptLine("Fra", 0, "+&2s	\"2s\"");
	processScriptLine("Fra", 0, "+&3s	\"3s\"");
	processScriptLine("Fra", 0, "+&13s	\"13s\"");
	processScriptLine("Fra", 0, "+&12s	\"12s\"");
	processScriptLine("Fra", 0, "+&m	\"m\"");
	processScriptLine("Fra", 0, "+&f	\"f\"");
	processScriptLine("Fra", 0, "+&pl +-PL	\"PL\"");
	processScriptLine("Fra", 0, "+&sg +-SG	\"SG\"");
	processScriptLine("Fra", 0, "+&f,&pl	\"f&pl\"");
	processScriptLine("Fra", 0, "+&m,&pl	\"m&pl\"");
	processScriptLine("Fra", 0, "+&f,&sg	\"f&sg\"");
	processScriptLine("Fra", 0, "+&m,&sg	\"m&sg\"");
	...
	processScriptLine("Fra", 0, "+|pro:subj	\"pro:subj\"");
...
static void processSpa(void) {
	...
	processScriptLine("Spa", 0, "+&IMP	\"*&IMP\"");
	processScriptLine("Spa", 0, "+-PROG	\"*-PROG\"");
	processScriptLine("Spa", 0, "+&PRES	\"*&PRES\"");
	processScriptLine("Spa", 0, "+&PRET	\"*&PRET\"");
	processScriptLine("Spa", 0, "+&FUT	\"*&FUT\"");
	processScriptLine("Spa", 0, "+&COND	\"*&COND\"");
	processScriptLine("Spa", 0, "+&SUB,&PRES	\"*&SUB&PRES\"");
	processScriptLine("Spa", 0, "+&SUB,&PAS	\"*&SUB&PAS\"");
...
static void processZho(void) {
	...
	processScriptLine("Zho", 0, "+|adj	\"adj\"");
	processScriptLine("Zho", 0, "+|cl	\"cl\"");
	processScriptLine("Zho", 0, "+|conn	\"conn\"");
	processScriptLine("Zho", 0, "+|det	\"det\"");
	processScriptLine("Zho", 0, "+|loc	\"loc\"");
	processScriptLine("Zho", 0, "+|n	\"n\"");
	processScriptLine("Zho", 0, "+|num	\"num\"");
	processScriptLine("Zho", 0, "+|pro	\"pro\"");
	processScriptLine("Zho", 0, "+|sfp	\"sfp\"");
	processScriptLine("Zho", 0, "+|v	\"v\"");
	processScriptLine("Zho", 0, "+|v:aux	\"v:aux\"");
	processScriptLine("Zho", 0, "+|wh	\"wh\"");
	processScriptLine("Zho", 0, "+|v:dirc	\"v:dirc\"");
	processScriptLine("Zho", 0, "+&DIM	\"DIM\"");
}
...
void init(char first) {
	...
	if (first) {
		...
		if (ftime) {
			...
			if (DBKeyRoot != NULL && strcmp(script_file, "eng") && strcmp(script_file, "zho")) {
				fprintf(stderr,"The database can only be used with \"eng\" or \"zho\" languages.\n");
				cutt_exit(0);
			}
			...
			if (isCreateDB) {
				if (strcmp(script_file, "eng") && strcmp(script_file, "zho")) {
					fprintf(stderr,"The database can only be used with \"eng\" or \"zho\" languages.\n");
					cutt_exit(0);
				}
...
static int fillUpEngFpFilePaths(int argc, char *argv[]) {
	...
	argc = addFilePath("childes-data/Eng-NA/Bloom70/*.cha", argc, argv);
	argc = addFilePath("childes-data/Eng-NA/Bloom73/*.cha", argc, argv);
...
//	argc = addFilePath("childes-data/Eng-NA/Kuczaj/*.cha", argc, argv);
		...
		if (isCreateDB) {
			if (!isLangGiven)
				...
				if (cOption == NULL) {
					...
					cutt_exit(0);
				} else if (strcmp(cOption, "_fp") == 0 || strcmp(cOption, "_md") == 0) {
		...
		case 's':
			if (*f == '[') {
				if (*(f+1) == '-') {
					...
					fprintf(stderr, "Please specify only precodes \"[- ...]\" with +/-s option.");
					cutt_exit(0);
				}
			} else {
