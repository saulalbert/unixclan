...
struct eval_speakers {
	...
	float density;	// CPIDR
	float noun, verb, aux, prep, adj, adv, conj, pron, det, thrS, thrnS, past, pastp, pl, presp;
	float retr, repeat;
...
struct database {
	...
	float mWords_sqr, mWords, mn_mWords, mx_mWords;
...
static void init_db(struct database *p) {
	...
	p->mWords_sqr = (float)0.0; p->mWords = (float)0.0; p->mn_mWords = (float)0.0; p->mx_mWords = (float)0.0;
...
static void prDebug(struct eval_speakers *ts) {
	...
	fprintf(stdout, "mWords=%.0f\n", ts->mWords);
	fprintf(stdout, "morf=%.0f\n", ts->morf);
	fprintf(stdout, "tUtt=%.0f\n", ts->tUtt);
	fprintf(stdout, "mUtt=%.0f\n", ts->mUtt);
...
static void eval_pr_result(void) {
	...
	excelCommasStrCell(fpout, "Language,Corpus,Code,Age,Sex,Group,SES,Role,Education,Custom_field");
	excelCommasStrCell(fpout, "Duration (sec),Total Utts,MLU Utts,MLU Words,MLU Morphemes");
	excelCommasStrCell(fpout, "FREQ types,FREQ tokens,FREQ TTR,Words/Min,Verbs/Utt");
	if (!isRawVal) {
		excelCommasStrCell(fpout, "% Word Errors,Utt Errors");
	} else {
		excelCommasStrCell(fpout, "Word Errors,Utt Errors");
	}
	...
	if (!isRawVal) {
		...
		excelCommasStrCell(fpout, "% Verbs,% Aux,% 3S,% 1S/3S,% PAST,% PASTP,% PRESP");
	...
	excelCommasStrCell(fpout, "noun/verb,open/closed");
...
//		Secs2Str(ts->tm, st, FALSE);
		...
		if (ts->mUtt == 0.0)
			excelCommasStrCell(fpout, "N/A,N/A");
		else {
...
//		excelNumCell(fpout, "%.0f" ,ts->morTotal); // comment to remove X column
		...
		if (DBKeyRoot != NULL) {
			...
			compute_SD(&SD[SDn++], ts->mUtt,  NULL, db->mUtt_sqr, db->mUtt, num, NULL);
			compute_SD(&SD[SDn++], ts->mWords,  &ts->mUtt, db->mWords_sqr, db->mWords, num, NULL);
			compute_SD(&SD[SDn++], ts->morf,  &ts->mUtt, db->morf_sqr, db->morf, num, NULL);
...
//			compute_SD(&SD[SDn++], ts->morTotal,  NULL, db->morTotal_sqr, db->morTotal, num, NULL); // comment to remove X column
	...
	if (DBKeyRoot != NULL) {
		...
		excelNumCell(fpout, "%.3f",db->mUtt/num);
		excelNumCell(fpout, "%.3f", db->mWords/num);
...
//		excelNumCell(fpout, "%.3f", db->morTotal/num); // comment to remove X column
		...
		excelNumCell(fpout, "%.0f", db->mn_mUtt);
		excelNumCell(fpout, "%.3f", db->mn_mWords);
...
//		excelNumCell(fpout, "%.0f", db->mn_morTotal); // comment to remove X column
		...
		excelNumCell(fpout, "%.0f", db->mx_mUtt);
		excelNumCell(fpout, "%.3f", db->mx_mWords);
