...
void init(char first) {
	...
	if (first) {
		...
		if (onlydata == 7) {
			if (isWOptUsed) {
				fputs("+w option can't be used with +d6 option.\n",stderr);
				cutt_exit(0);
			}
...
static void outputExcel(void) {
	...
	if (StatFpOut != NULL) {
		...
		fputs("File,Language,Corpus,Speaker,Age,Sex,Group,SES,Role,Education,Custom field,# matches,# utts,matches/utts\n", StatFpOut);
		for (ts=combo_head; ts != NULL; ts=ts->next_sp) {
			if (ts->isFound) {
				fprintf(StatFpOut, "%s", ts->fname);
				outputIDForExcel(StatFpOut, ts->ID, -1);
				fprintf(StatFpOut, ",%.0f,%.0f,%.3f\n", ts->spMatch, ts->spUtts, ts->spMatch/ts->spUtts);
			}
		}
		if (!f_override) {
			if (Preserve_dir) {
...
static void combo_pr_result(void) {
	if (isIncludeTier == TRUE && onlydata != 7) {
		if (onlydata == 0)
...
static int CMatch(char *s, PAT_ELEM *tm, char wild) { /* return index of pat in s, -1 if no Match */
	register int i, j;
...
static int findmatch(char *txt) {
	...
	char *outLine;
	char wild, isMatched, isStop, isNegMatched = FALSE;
	struct cdepTiers *t;
	PAT_ELEM *flatp;
...
static void combineMainDepWords(char *line) {
	int i;
...
*/
			...
			if (linkDep2Other) {
				if (strchr(uttline, dMarkChr) != NULL) {
					combineMainDepWords(utterance->line);
					combineMainDepWords(uttline);
				}
		...
		case 'd':
				if (*f == 'v' || *f == 'V')
					...
					if (*f != EOS && !isdigit(*f)) {
						...
						fprintf(stderr, "The only +d levels allowed are 0-7.\n", OnlydataLimit-1);
						cutt_exit(0);
					}
