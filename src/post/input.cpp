...
void readConllFile(FNType *dbname) {
	...
	while (fgets(templineC, UTTLINELEN, fp)) {
		...
		if (uS.isUTF8(templineC) || uS.partcmp(templineC,"@Font:",0,0) || uS.partcmp(templineC,CKEYWORDHEADER,0,0))
#else
...
#ifdef POSTCODE
				if (!uS.partcmp(IOs[ID].linebuffer, PIDHEADER, FALSE, TRUE) && !uS.partcmp(IOs[ID].linebuffer, CKEYWORDHEADER, FALSE, TRUE)&&
					!uS.partcmp(IOs[ID].linebuffer,FONTHEADER, FALSE, TRUE) && !uS.isUTF8(IOs[ID].linebuffer))
#else
				if (!partcmp(IOs[ID].linebuffer, "@PID:", FALSE, TRUE) && !partcmp(IOs[ID].linebuffer, "@Color words:", FALSE, TRUE)&&
					!partcmp(IOs[ID].linebuffer,"@Font:", FALSE, TRUE) && !isUTF8(IOs[ID].linebuffer))
#endif
					ln++;
