...
static int  fileID;
static char ipsyn_ftime, isAndSymFound, isVerbose;
...
static UTT_WORDS *match(UTT_WORDS *utt_word, PAT_ELEMS *tm, char wild, char *isStillMatched, char isTrueAnd, UTT_WORDS *bWord, TEMPMATCH *tempMatch) {
	char isNegFound;
	...
	if (tm->pat_elem != NULL) {
		...
		if ((tmp=match(utt_word,tm->parenElem,wild,isStillMatched,isTrueAnd,bWord,tempMatch)) != NULL) {
			utt_word = tmp;
			if (tm->neg) {
				if (isTrueAnd)
					return(NULL);
	...
	if (tm->andElem != NULL) {
		if (negSf != NULL || negSl != NULL) {
			if (negSf != NULL) {
				tUtt_word = utt_word;
				utt_word = match(utt_word,tm->andElem,tm->wild,isStillMatched,isTrueAnd,bWord,tempMatch);
...
static char addDummyToSpResRules(RES_RULES *resRules, int point, long ln) {
	UTT_WORDS *p;

	if (point == 1) {
		resRules->ln1 = ln;
		p = addUtt_words(resRules->point1, NULL, NULL, NULL, NULL, '\0', TRUE); // adding DUMMY element
		if (p == NULL) {
			fprintf(stderr,"Error: out of memory\n");
			return(FALSE);
		}
		...
		resRules->ln2 = ln;
		p = addUtt_words(resRules->point2, NULL, NULL, NULL, NULL, '\0', TRUE); // adding DUMMY element
		if (p == NULL) {
			fprintf(stderr,"Error: out of memory\n");
			return(FALSE);
		}
	...
	for (flatp=rule_cond->exclude; flatp != NULL; flatp=flatp->orElem) {
		...
		while (utt_word != NULL && utt_word->isLastStop == FALSE) {
			...
			if (flatp->isAllNeg) {
				...
				utt_word = match(utt_word,flatp,wild,&isStillMatched,rule->isTrueAnd,bWord,&tempMatch);
...
static int getAdditionalScore(RULES_LIST *rule, RES_RULES *resRules, char *isErr) {
	int cnt, cntMatched;
	IFLST *ifs;
	ADDLST *adds;
	RULES_LIST *tRule;
							...
							for (flatp=rule_cond->include; flatp != NULL; flatp=flatp->orElem) {
								while (utt_word != NULL && utt_word->isLastStop == FALSE) {
									...
									if (flatp->isAllNeg) {
										...
										utt_word = match(utt_word,flatp,wild,&isStillMatched,rule->isTrueAnd,orgUtt_word,&tempMatch);
										if (utt_word != NULL && isStillMatched) {
											isErr = FALSE;
											...
											if (resRule != NULL) {
												if (!addDummyToSpResRules(resRule, point, utt->ln))
													return(FALSE);
			...
			for (resRule=sp->resRules; resRule != NULL; resRule=resRule->next_result) {
				if (resRule->rule == rule && (resRule->point1 != NULL || resRule->point2 != NULL)) {
					...
					if (rule->adds == NULL)
						...
						isErr = FALSE;
						addScore = getAdditionalScore(rule, sp->resRules, &isErr);
						if (isErr) {
							return(FALSE);
						}
					}
					if (isOutput && addScore > 0)
						fprintf(fpout, "    ADD: score: %d\n", addScore);
...
char isExcludeIpsynTier(char *line, char *tline) {
	char *sp, isUttDelFound;
...
static RULES_LIST *createNewRule(int ln, char *name, char *isErr) {
	RULES_LIST *rule;
...
static char read_ipsyn(char *lang) {
	...
	while (fgets_cr(templineC, UTTLINELEN, fp)) {
		if (uS.isUTF8(templineC) || uS.partcmp(templineC, FONTHEADER, FALSE, FALSE) ||
			uS.partcmp(templineC, PIDHEADER, FALSE, FALSE) ||
