...
static int check_matchSpeaker(SPLIST *tp, char *s) {
...
static char check_OverAll(void) {
	...
	while (!feof(fpin)) {
		...
		if (uttline[i] == '\n' || feof(fpin)) {
			uttline[i] = EOS;
			if (uS.partcmp(uttline, PIDHEADER, FALSE, TRUE)) {
				i = 0;
				continue;
			}
...
*/
						...
						check_err(48, pos+s-1, pos+s-1, ln);
						   ...
						   (s == 0 || (w[s-1] != '-' && w[s-1] != '\'' && w[s-1] != '_' && w[s-1] != '0' && w[s-1] != '(' && s != CAOffset)) && !capWErrFound) {
		...
		if (w[0] == (char)0xe2 && w[1] == (char)0x80 && w[2] == (char)0x9E && w[3] == EOS) {
			...
			while (w[s] != EOS) {
				if (isalnum((unsigned char)w[s]) || my_CharacterByteType(w, (short)s, &dFnt) != 0) {
					...
					if ((t=isBadQuotes(w, s)) > 0) {
						check_err(t, pos+s-1, pos+s+t-2, ln);
						s += 3;
					} else if (isIllegalASCII(w, s)) {
						check_err(86, pos+s-1, pos+s-1, ln);
					} else if (w[s] == HIDEN_C) {
	...
	while (1) {
		...
		*st = EOS;
		if (word[0] == HIDEN_C) {
			...
			if (utterance->speaker[0] == '*' || check_isBlob == FALSE) {
...
//		if (!nomap) uS.uppercasestr(word, &dFnt, MBF);
		...
		if (word[0] == HIDEN_C) {
			...
			if (!check_isPlayBullet(word)) {
				isTextBetweenBulletsFound = TRUE;
			} else if ((utterance->speaker[0] == '*' || check_isBlob == FALSE) && (!check_isMultiBullets || check_lastBulletOnTier(uttline+e))) {
				if ((res=check_checkBulletsConsist(&FirstBulletFound, word)) != 0)
					check_err(res,s-1,s-1,ln+lineno);
				isTextBetweenBulletsFound = FALSE;
			}
...
*/
		...
		if (*utterance->speaker == '@') {
			if (uS.partcmp(utterance->speaker, PIDHEADER, FALSE, TRUE))
				continue;
			...
			if (uS.patmat(templineC, "@Bg")) {
				...
				while (uttline[i]) {
					...
					if (!strcmp(uttline+i, "zh")  || !strncmp(uttline+i, "zh-", 3)  ||
						...
						!strcmp(uttline+i, "min") || !strcmp(uttline+i, "hak"))
...
*/
			...
			if (uS.patmat(templineC,PARTICIPANTS))
				...
				else if (uS.patmat(templineC,SESOF)) check_ses(ts, templineC, uttline, 0, EOS);
