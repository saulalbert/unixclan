...
void usage() {
	...
	puts("+c5: when combined with +d7 option it will reverse tier's priority (default: first tier is on top)");
...
static void printCT(struct freq_tnode *p, ColLabels *colList) {
	...
	if (p != NULL) {
		...
		do {
			outputStringForExcel(fpout, p->word, 1);
...
static void freq_pr_result(void) {
	...
	if (isCTSpreadsheet) {
		...
		for (ts=freq_head; ts != NULL; ts=ts->next_sp) {
			...
			if (isMorTierFirst)
				fprintf(fpout, "%s,Total %s,", fDepTierName+1, fDepTierName+1);
			else
				fprintf(fpout, "%s,Total %s,", sDepTierName+1, sDepTierName+1);
			for (cl=ts->colList; cl != NULL; cl=cl->nextCol)
				fprintf(fpout, "%s,", cl->colName);
			fprintf(fpout, "\n");
			printCT(ts->words, ts->colList);
			fprintf(fpout, "%s,,", "Total Cols");
			for (cl=ts->colList; cl != NULL; cl=cl->nextCol)
				fprintf(fpout, "%d,", cl->totalCol);
			fprintf(fpout, "\n");
			fprintf(fpout, "\n");
		}
	} else {
...
*/
		...
		if (!chatmode) {
			if (strncmp(uttline, ";%* ", 4) == 0 || uS.isUTF8(uttline) || uS.partcmp(uttline, FONTHEADER, FALSE, FALSE))
				continue;
...
static void statfreq_percent_result(int percentNum, statfreq_sp_list *sp, FILE *fp) {
	...
	tisLookAtSpeakerWord = isLookAtSpeakerWord;
	isLookAtSpeakerWord = TRUE;
	fprintf(fp, "%c%c%c", 0xef, 0xbb, 0xbf); // BOM UTF8
	if (onlydata == 3) {
		fputs("File,Language,Corpus,Speaker,Age,Sex,Group,SES,Role,Education,Custom field,", fp);
		statfreq_print_percent_row(NULL, WordsHead, percentNum, fp, NULL, NULL);
		fprintf(fp, "Types,Token,TTR");
		putc('\n', fp);
		if (!isMorUsed) {
			fprintf(fp, "\"This TTR number was not calculated on the basis of %%mor line forms.\"\n");
			fprintf(fp, "\"If you want a TTR based on lemmas, run FREQ on the %%mor line\"\n");
			fprintf(fp, "\"with option: +sm;*,o%\"\n");
		}
		while (sp != NULL) {
			outputStringForExcel(fp, sp->fname, 1);
			outputIDForExcel(fp, sp->ID, 1);
			total = 0.0;
			diff = 0.0;
			statfreq_print_percent_row(sp, WordsHead, percentNum, fp, &diff, &total);
			fprintf(fp,"%.0f,%.0f,", diff, total);
			if (total != 0)
				fprintf(fp,"%.3f", diff/total);
			else
				fprintf(fp,"-");
			putc('\n', fp);
			sp = sp->next_sp;
...
static void statfreq_pr_result(char *fname, char *id, char *stats, FILE *fp) {
	outputStringForExcel(fp, fname, 1);
	outputIDForExcel(fp, id, 1);
...
//				OverWriteFile = FALSE;
				...
				if (StatFpOut != NULL) {
					...
					if (frame_size > 0)
						fprintf(StatFpOut, "Types,Token,TTR,MATTR");
					else
						fprintf(StatFpOut, "Types,Token,TTR");
					putc('\n', StatFpOut);
					if (!isMorUsed) {
						fprintf(StatFpOut, "\"This TTR number was not calculated on the basis of %%mor line forms.\"\n");
						fprintf(StatFpOut, "\"If you want a TTR based on lemmas, run FREQ on the %%mor line\"\n");
						fprintf(StatFpOut, "\"with option: +sm;*,o%\"\n");
					}
