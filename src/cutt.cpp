...
static char isSearchMorSym; /* 1 if user searches for symbols: cm|*, end|*, beg|*, bq|*, eq|* */
...
static char org_depTier[UTTLINELEN+2];/* used by createMorUttline function for compairing two dep tiers*/
...
static char tempTier[UTTLINELEN+2];   /* used by createMorUttline, remove_main_tier_print, cutt_cleanUpLine->expandX */
static char tempTier1[UTTLINELEN+2];  /* used by getmaincode */
...
// 2014-07-26 static char isMorWordSpec = 0;	/* if %mor word spec with +s, then 1, otherwise 0 */
// 2014-07-26 static char isPlainWordSpec = 0;/* if speaker plain word spec with +s, then 1, otherwise 0 */
...
static char isIxesMatchPat(IXXS *ixes, int max, char *pat) {
	char isMatched;
	int i;
...
static MORFEATURESLIST *makeMorSearchFeatList(char *wd, char *orgWd, char *isClitic, char *res) {
	...
	if ((tItem=NEW(MORFEATURESLIST)) == NULL) {
		...
		if ((tItem->typeID == '~' || tItem->typeID == '$') && *wd == '%') {
			...
			if (CLAN_PROG_NUM != KIDEVAL && CLAN_PROG_NUM != IPSYN && CLAN_PROG_NUM != MORTABLE && CLAN_PROG_NUM != FLUCALC) {
				if (!isSearchMorSym) {
...
static int ExcludeMORScope(char *wline, int pos, char isblankit, char isReplaceWord) {
	if (isReplaceWord) {
		if (CLAN_PROG_NUM != FREQ && CLAN_PROG_NUM != FREQMERGE && CLAN_PROG_NUM != FREQPOS)
			isReplaceWord = FALSE;
	}
	if (!uS.isRightChar(wline, pos, '[', &dFnt, MBF) && pos >= 0) {
		while (!uS.isRightChar(wline, pos, '[', &dFnt, MBF) && pos >= 0) {
...
static int excludeSpTierMorDef(char *word) {
	...
	if (uS.mStricmp(word, "xx") == 0 || uS.mStricmp(word, "xxx") == 0 || strcmp(word, "0") == 0 ||
		...
		word[0] == '#' || word[0] == '-' || word[0] == '[' || uS.patmat(word, "(*.*)")) {
		return(FALSE);
	} else if (word[0] == '&') {
		for (twd=defwdptr; twd != NULL; twd = twd->nextword) {
			if (uS.patmat(word, twd->word)) {
...
static void copyFrontFillersToDepTier(char *spTier, char *mor_tier) {
	int  i, j, t, repWord;
	char spWord[BUFSIZ], MorWord[BUFSIZ], isMorDone, res;
...
void createMorUttline(char *new_mor_tier, char *spTier, const char *dcode, char *mor_tier, char isFilterSP, char linkTiers) {
	...
	if (isFilterSP) {
		...
		if (linkTiers && CLAN_PROG_NUM == COMBO && strchr(spTier, '&') != NULL) {
			copyFrontFillersToDepTier(spTier, mor_tier);
		}
	}
...
*/
	...
	isRemoveCAChar[NOTCA_OPEN_QUOTE]    = tOQ;
	isRemoveCAChar[NOTCA_CLOSE_QUOTE]   = tCQ;
...
char isnEqualIxes(const char *pat, IXXS *ixes, int max, int len) {
	int i;
	...
	for (i=0; i < max; i++) {
		if (ixes[i].ix_s != NULL) {
			if (!uS.mStrnicmp(ixes[i].ix_s, pat, len))
				return(TRUE);
		}
	}
	return(FALSE);
...
/**************************************************************/
/* isPostCodeOnUtt(str) determines if the code token in "str" is to be included
	...
	for (pos=0; line[pos] != EOS; pos++) {
		if (line[pos] == '[' && line[pos+1] == '+') {
			...
			if (line[pos] == EOS)
				return(FALSE);
			templineC2[i] = EOS;
			removeExtraSpace(templineC2);
			if (uS.mStricmp(templineC2, postcode) == 0)
				return(TRUE);
		}
	}
	return(FALSE);
...
void mmaininit(void) {
	...
	linkMain2Mor = FALSE;
	isExpendX = TRUE;
// 2014-07-26 	isMorWordSpec = 0;
// 2014-07-26 	isPlainWordSpec = 0;
	anyMultiOrder = FALSE;
	...
	if (CLAN_PROG_NUM == MLT || CLAN_PROG_NUM == MLU || CLAN_PROG_NUM == MODREP || CLAN_PROG_NUM == REPEAT ||
		...
		CLAN_PROG_NUM == JOINITEMS || CLAN_PROG_NUM==SYNCODING || CLAN_PROG_NUM==CMDI_P || CLAN_PROG_NUM==LAB2CHAT ||
		CLAN_PROG_NUM == MORTABLE || CLAN_PROG_NUM == FIXLANG || CLAN_PROG_NUM == SUBTITLES || CLAN_PROG_NUM == IPSYN ||
		CLAN_PROG_NUM == EVAL || CLAN_PROG_NUM == KIDEVAL || CLAN_PROG_NUM == MAXWD || CLAN_PROG_NUM == SCRIPT_P || CLAN_PROG_NUM == TIMEDUR || CLAN_PROG_NUM == WDLEN ||
		CLAN_PROG_NUM == CHAT2CONLL || CLAN_PROG_NUM == CONLL2CHAT || CLAN_PROG_NUM == FLUCALC || CLAN_PROG_NUM == CNLA)
		R6 = FALSE;
	else
		R6 = TRUE;
	R6_override = FALSE;
	...
	if (CLAN_PROG_NUM == FREQ || CLAN_PROG_NUM == FREQMERGE || CLAN_PROG_NUM == DSS || CLAN_PROG_NUM == PHONFREQ ||
		...
		CLAN_PROG_NUM == CMDI_P || CLAN_PROG_NUM == LAB2CHAT || CLAN_PROG_NUM == MORTABLE || CLAN_PROG_NUM == FIXLANG ||
		CLAN_PROG_NUM == SUBTITLES || CLAN_PROG_NUM == FLO || CLAN_PROG_NUM == IPSYN || CLAN_PROG_NUM == SCRIPT_P ||
		CLAN_PROG_NUM == TIMEDUR || CLAN_PROG_NUM == EVAL || CLAN_PROG_NUM == KIDEVAL || CLAN_PROG_NUM == CHAT2CONLL ||
		CLAN_PROG_NUM == CONLL2CHAT || CLAN_PROG_NUM == FLUCALC || CLAN_PROG_NUM == VOCD || CLAN_PROG_NUM == CHIP ||
		CLAN_PROG_NUM == CNLA)
		nomap = TRUE;
	else
		nomap = FALSE;
...
void InitOptions(void) {
	...
	option_flags[CNLA] = F_OPTION+RE_OPTION+SM_OPTION+T_OPTION+UP_OPTION;
	strncpy(options_ext[CNLA], ".cnl", OPTIONS_EXT_LEN); options_ext[CNLA][OPTIONS_EXT_LEN] = EOS;
	...
	option_flags[FREQMERGE] = F_OPTION+RE_OPTION+K_OPTION+SP_OPTION+SM_OPTION+P_OPTION;
	...
	option_flags[IPSYN] = F_OPTION+RE_OPTION+T_OPTION+L_OPTION+UP_OPTION;
	...
	option_flags[MEGRASP] = RE_OPTION+FR_OPTION+SP_OPTION+SM_OPTION;
	...
	option_flags[ANGLES]= F_OPTION+RE_OPTION+FR_OPTION;
	strncpy(options_ext[ANGLES], ".angle", OPTIONS_EXT_LEN); options_ext[ANGLES][OPTIONS_EXT_LEN] = EOS;
	...
	option_flags[TEMP02] = F_OPTION;
	strncpy(options_ext[TEMP02], ".comb", OPTIONS_EXT_LEN); options_ext[TEMP02][OPTIONS_EXT_LEN] = EOS;
	...
	option_flags[TEMP] = F_OPTION+FR_OPTION+RE_OPTION;
...
char *mainflgs() {
	*templineC = EOS;
	...
	if (option_flags[CLAN_PROG_NUM] & SP_OPTION || option_flags[CLAN_PROG_NUM] & SM_OPTION) {
		...
		} else if (CLAN_PROG_NUM == EVAL || CLAN_PROG_NUM == CNLA) {
			strcat(templineC,"sS ");
		} else {
...
//			puts("+d41: outputs in SPREADSHEET format with each matched keyword listed one per column");
	...
	if (option_flags[CLAN_PROG_NUM] & SP_OPTION) {
		if (CLAN_PROG_NUM == MOR_P || CLAN_PROG_NUM == POSTMORTEM || CLAN_PROG_NUM == MEGRASP) {
			puts("+sS: select language to analyze with \"[- ...]\" postcode.");
		} else if (CLAN_PROG_NUM == FLUCALC) {
		...
		} else if (CLAN_PROG_NUM == CNLA || CLAN_PROG_NUM == EVAL || CLAN_PROG_NUM == KIDEVAL) {
			puts("+sS: select utterance to analyze with \"[+ ...]\" postcode.");
		} else {
	...
	if (!combinput && ByTurn == '\0') {
		if (option_flags[CLAN_PROG_NUM] & UP_OPTION) {
			if (CLAN_PROG_NUM == QUOTES || CLAN_PROG_NUM == VOCD || CLAN_PROG_NUM == EVAL || CLAN_PROG_NUM == MORTABLE ||
				...
				CLAN_PROG_NUM == CHAT2CONLL || CLAN_PROG_NUM == FLUCALC || CLAN_PROG_NUM == CNLA)
...
*/
	...
	isUnicodeFont = (strcmp("Arial Unicode MS", dFnt.fontName) == 0 || strcmp("Arial", dFnt.fontName) == 0 || strcmp("CAfont", dFnt.fontName) == 0);
	strcpy(dFnt.fontName, finfo->fontName);
...
static void createLanguageTable(FILE *fp) {
	...
	while (fgets_cr(buf, BUFSIZ, fp)) {
		if (uS.isUTF8(buf) || uS.partcmp(buf,FONTHEADER,FALSE,FALSE) || buf[0] == '#')
			continue;
...
void outputStringForExcel(FILE *fp, char *st, short isComma) {
	if (isComma < 0)
		fprintf(fp, ",");
	if (strchr(st, ',')) {
		fprintf(fp, "\"");
...
static int work(char *argv[], int argc, FNType *fname) {
	...
	if (!combinput)
		WUCounter = 0L;
			 ...
			 CLAN_PROG_NUM == FLUCALC || CLAN_PROG_NUM == CNLA ||
...
//			getpunct(punctFile);
		...
		if (stin) {
			if (FirstTime) {
				if (CLAN_PROG_NUM != CHECK) { // 2017-04-11
			...
			if (CLAN_PROG_NUM != POST && CLAN_PROG_NUM != CMDI_P && CLAN_PROG_NUM != RELY && CLAN_PROG_NUM != CHECK) { // 2017-04-11
#if defined(CLAN_SRV)
...
#else // CLAN_SRV
				if (!stin_override) {
					if (CLAN_PROG_NUM != CNLA && CLAN_PROG_NUM != EVAL && CLAN_PROG_NUM != KIDEVAL && CLAN_PROG_NUM != FLUCALC) {
...
//				getpunct(punctFile);
			...
			if (stin) {
				...
				if (CLAN_PROG_NUM != CHECK) { // 2017-04-11
					...
					if (CLAN_PROG_NUM != RELY && CLAN_PROG_NUM != CNLA && CLAN_PROG_NUM != EVAL &&
						CLAN_PROG_NUM != KIDEVAL && CLAN_PROG_NUM != FLUCALC)
						fprintf(stderr,"From file <%s>\n",fname);
				}
				do {
					call();
				...
				if (!combinput && CLAN_PROG_NUM != RELY && CLAN_PROG_NUM != CHECK) { // 2017-04-11
					if (!stin_override)
						fprintf(stderr,"Output file <%s>\n",showFName);
					else
						fprintf(stderr,"Output file \"%s\"\n",showFName);
					...
					if (!WD_Not_Eq_OD) {
						...
						if (!access(tempfName, 0)) {
							if (unlink(tempfName)) {
								...
								do_warning(templineC3, 0);
#else
								fprintf(stderr, "Can't delete original file \"%s\". Perhaps it is opened by some application.", tempfName);
#endif
								if (IDField != NULL || CODEField != NULL || SPRole != NULL) {
									CleanUpTempIDSpeakers();
								}
...
//	NewFontInfo finfo;
	...
	res = TRUE;
#ifdef UNX
	if (CLAN_PROG_NUM == EVAL || CLAN_PROG_NUM == CNLA) {
		char isCreateDB = 0;
		for (i=1; i < argc; i++) {
			if (*argv[i] == '+'  || *argv[i] == '-') {
				if (argv[i][1] == 'c') {
		...
		if (*argv[i] == '+'  || *argv[i] == '-') {
			...
			if (argv[i][1] == 'k') {
				if (!nomap)
					...
					nomap = FALSE;
			} else if (argv[i][1] == 's' && argv[i][2] == 'm' && argv[i][3] == EOS) {
				morSearchSytaxUsage(argv[i]);
			} else if (argv[i][1] == 's' && argv[i][2] == 'g' && argv[i][3] == EOS) {
...
// new code 2015-02-19 begin
				...
				args.argc = argc;
				args.targ = targ;
				if (!cutt_dir(st,&args,&isFileFound,&worked,NULL))
					break;
				free_FileList(args.root_file);
				args.root_file = NULL;
...
//				isAddLang = TRUE;
				for (; ch[index] != EOS && !uS.isskip(ch,index,&dFnt,MBF); index++) {
					if (uS.isRightChar(ch, index, '(', &dFnt, MBF) && uS.isPause(ch, index, NULL,  &i)) {
						...
						index = index + i - 1;
					} else if (ch[index] == '@' && ch[index+1] == 's' && (ch[index+2] == ':' || ch[index+2] == '$' || uS.isskip(ch,index+2,&dFnt,MBF))) {
						oIndex = -1;
						index += 2;
						isAddLang = FALSE;
						if (ch[index] == '$') {
							...
							if (fDepTierName[0] != EOS && uS.partcmp(templineC,fDepTierName,FALSE,FALSE)) {
								if (uS.partcmp(fDepTierName,"%GRA",FALSE,FALSE) ||
									...
									if (linkDep2Other && CLAN_PROG_NUM == COMBO && strchr(org_spTier, '&') != NULL) {
										copyFrontFillersToDepTier(org_spTier, lutter->tuttline+posT);
									}
...
//					if (WordMode != 'e' && MorWordMode != 'e')
				...
				uttline = utterance->tuttline;
				if (uS.partcmp(utterance->line,PIDHEADER,FALSE,FALSE)) {
					tlineno--;
				} else if (uS.partcmp(utterance->line,CKEYWORDHEADER,FALSE,FALSE)) {
					tlineno--;
			...
			if (chatmode) {
				...
				if (uS.partcmp(templineC,"@ID:",FALSE,FALSE)) {
					...
					if (linkDep2Other) {
						...
						if (isMORSearch() || linkMain2Mor || (R8 && CLAN_PROG_NUM == FREQ)) {
